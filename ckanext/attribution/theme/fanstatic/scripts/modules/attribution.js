// css selector for the hidden input containing the agent form data
const hiddenSelector = '#dataset-edit input[name="agent"]';
...
function moveUp(el) {
    let allContrib    = allAgent();
    let sortedContrib = Object.values(allAgent()).sort((a, b) => a.order - b.order)
                              .map((c) => c.id);
...
$(document).ready(function () {
    ...
    function addRow(rowIndex, rowId, rowContent, skip, disableEdit) {
        ...
        newRowElem.find('.contributor-field input').on('change', (e) => {
            ...
            let currentContribList = allAgent();
    ...
    Object.entries(allAgent()).sort((a, b) => {
